...
static void wifi_event_handler( void *arg, esp_event_base_t event_base, i32 event_id, void *event_data ) {
  ...
  /*
 *
 * thought i was going to have to use a hashtable to associate stations with ips etc.
 * turns out there actually is an undocumented API function to get the station list with assigned IPs. */
  ...
  if ( event_id = IP_EVENT_AP_STAIPASSIGNED ) {
    ...
    // 0 = immediately return if queue ful.
    xQueueSendToFront( peer_ip_queue, station_ip, 0 );
  }
...
// register event handler with base types of both wifi and ip 
...
int reconnect_callback( TimerHandle_t timer ) {
  ...
  for ( int i=0; i < NUM_FRIEND_IDS; i++ ) {
    for ( int j=0; j < 20; j++ ) {
      ...
      if ( strncmp( friends_list[i].ssid, ssid, 32 ) == 0 ) {
  ...
  while ( ;; ) {
    switch( xEventGroupGetBits( wifi_event_group ) ) {
      ...
      case WIFI_EVENT_CONNECTED:
        ...
        esp_netif_get_sta_list(&wifi_sta_list, &tcpip_sta_list);
        
        for ( int i=0; i < netif_sta_list.num; i++ ) {

          esp_netif_sta_info_t station = netif_sta_list[i];
          // item is queued by copy
          xQueueSend( peer_ip_queue, &station, 0 );
